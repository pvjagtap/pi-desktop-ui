...
graph LR
    subgraph "Extension → Window Messages"
        EXT["Extension sends:"]
        EXT --> B1["agent-start / agent-end"]
        EXT --> B2["message-start / chunk / end"]
        EXT --> B3["thinking-start / chunk / end"]
        EXT --> B4["tool-start / tool-end"]
        EXT --> B5["stats-update"]
        EXT --> B6["thread-messages"]
        EXT --> B7["explorer-data / tree-children"]
        EXT --> B8["file-content"]
        EXT --> B9["commands-list"]
        EXT --> B10["update-threads / update-skills"]
        EXT --> B11["workspaces-list / workspace-sessions"]
        EXT --> B12["search-results"]
        EXT --> B13["workspace-opened"]
        EXT --> B14["file-attached-ack"]
        EXT --> B15["plan-mode-violation"]
        EXT --> B16["session-changed"]
        EXT --> B17["provider-request"]
    end
```
